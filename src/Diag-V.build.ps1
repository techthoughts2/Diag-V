...
Add-BuildTask ValidateRequirements {
    #running at least powershell 5.1?
    Assert-Build ($PSVersionTable.PSVersion -ge [version]5.1) 'At least Powershell 5.1 is required for this build to function properly'
}#ValidateRequirements
...
Add-BuildTask CreateMarkdownHelp -After CreateHelpStart {
    ...
    Write-Build Gray '               ...Taking additional formatting steps to account for Diag-V having a dash in name.'
    # $helpDocFiles = Get-ChildItem -Path $OutputDir
